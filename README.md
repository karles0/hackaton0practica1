...
- **Objetivo**: Crear y configurar un repositorio en GitHub que refleje buenas prácticas de desarrollo colaborativo.
  - **Tareas**:
    1. Crear un repositorio privado en GitHub. El admin de dicho repositorio deberá añadir a todos los miembros del equipo.
    2. Crear un archivo `README.md` en el directorio `docs/` (vacío por ahora).
    3. En base a los videos tutoriales de la semana 0 el usuario admin del repo deberá configurar las reglas de la rama principal (`main` o `master`) para que no se pueda hacer `push` directo a esta rama. Los cambios en la rama principal solo deben realizarse mediante **pull requests aprobados**.
    4. Crear un archivo `.gitignore` en el root del repositorio y añadir los directorios y archivos que no deben ser versionados, por ejemplo, `__pycache__/`, `*.pyc`, `.env`, `venv/`, `.venv/`, etc.
  - **Evaluación**: Mostrar el repositorio y capturas de pantalla de la configuración de las reglas de la rama principal.
...
- **Objetivo**: Trabajar con issues, pull requests, y resolver conflictos en el merge mientras se desarrolla un mini proyecto en Python.
  ...
  - **Evaluación**:
    ...
    - La implementación debe ejecutar correctamente los test incluidos en `tests/test.py`.
    - Para ejecutar las pruebas de Python, usa el siguiente comando:      
        ```sh
...
- **Fecha Límite**: La hackathon deberá completarse antes del 26 de agosto a las 20 horas, momento en el cual los repositorios serán revisados.
